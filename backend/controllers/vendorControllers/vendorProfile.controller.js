...
exports.updateAccount = async (req, res) => {
  try {
    ...
    const updatedVendor = await vendorProfileServices.updateVendorAccount(vendorId,req.body);

    // if request successfully handled
    res.status(200).json({ message: "Profile updated successfully", updatedVendor});
  } catch (error) {
    // if any error occurs
    res
      .status(error.statusCode || 500)
